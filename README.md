...
1. Introduction
This system is designed to help users upload documents (e.g., Excel FAQs, PDF manuals), convert them into searchable knowledge using embeddings + Chroma vector DB, and then ask natural language questions via a web interface (FastAPI/Streamlit UI).
...
Creates a structured Excel output (e.g., generated_user_manual.xlsx) with Q&A results
...
Backend: FastAPI server with endpoints /upload, /ask, /health, /metrics
...
Database not loading → Delete old chroma_db_excel folder and restart server
Wrong language output → Update Excel FAQ to English or enable translator integration
